...
const DiabetesCareDetail = () => {
  ...
  // Increase qty for similar product
  const simInc = (id) => {
    setSimQty((prev) => ({ ...prev, [id]: (prev[id] || 0) + 1 }));
  };

  // Decrease qty for similar product
  const simDec = (id) => {
    setSimQty((prev) => ({
      ...prev,
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8">
      ...
      <div className="mt-10 bg-white border p-8 rounded-xl shadow-sm">
        ...
        <p>
          <b>Product Type:</b> {product.highlights?.product_type || "-"}
        </p>
        <p>
          <b>Pack Size:</b> {product.highlights?.pack_size || "-"}
        </p>
        <p>
          <b>Shelf Life:</b> {product.highlights?.shelf_life || "-"}
        </p>
        <p>
      ...
      <div className="mt-12 relative">
        ...
        <div
          ref={scrollRef}
          className="flex overflow-x-auto gap-4 no-scrollbar scroll-smooth pb-3"
        >
          {allProducts.slice(0, 12).map((sim) => {
            ...
            return (
              <Link
                ...
                className="min-w-[220px] block"
              >
                <div className="bg-white rounded-2xl shadow-md border p-4 relative hover:shadow-lg transition">
                  {/* DISCOUNT BADGE */}
                  {sim.cost > sim.final_price && (
                    <div className="absolute top-2 left-2 bg-green-600 text-white text-xs font-bold px-2 py-1 rounded-md shadow">
                      Save{" "}
                      {Math.round(
                        ((sim.cost - sim.final_price) / sim.cost) * 100
                      )}
                      %
                    </div>
                  )}
                  ...
                  {/* IMAGE */}
                  <div className="w-full h-28 flex justify-center items-center mt-3">
                    <img
                      src={simImg}
                      alt={sim.name}
                  ...
                  <div className="mt-4">
                    ...
                    <p className="text-green-700 font-semibold text-sm mt-1">
                      ₹{price(sim.cost - sim.final_price)} OFF
                    </p>
                  </div>
                  ...
                  <p className="text-gray-500 text-xs">
                    {sim.highlights?.pack_size || "200 ml"}
                  </p>
                  ...
                  {simQty[sim.id] ? (
                    <div className="absolute bottom-3 right-3 bg-white border border-green-600 rounded-xl shadow-sm flex items-center px-3 py-1">
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          simDec(sim.id);
                    ...
                    <button
                      ...
                      className="absolute bottom-3 right-3 bg-orange-500 text-white px-4 py-1 rounded-xl font-bold text-sm shadow-sm"
                    >
                      ADD
                    </button>
                  )}
...
>
  <div className="bg-white rounded-2xl shadow-md border p-4 relative hover:shadow-lg transition">
    ...
    {sim.cost > sim.final_price && (
      <div className="absolute top-2 left-2 bg-green-600 text-white text-xs font-bold px-2 py-1 rounded-md shadow">
        Save {Math.round(((sim.cost - sim.final_price) / sim.cost) * 100)}%
      </div>
    )}
    ...
    {/* RATING */}
    <div className="flex items-center text-sm mt-3">
      <span className="text-green-700 font-bold">⭐ {sim.rating}</span>
      <span className="text-gray-500 text-xs ml-1">(1.7k)</span>
    </div>
    ...
    {simQty[sim.id] ? (
      <div className="absolute bottom-3 right-3 bg-white border border-green-600 rounded-xl shadow-sm flex items-center px-3 py-1">
        ...
        <span className="px-2 font-semibold">{simQty[sim.id]}</span>
